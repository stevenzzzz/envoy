...
// Test ScopedRoutesConfigProvider that returns a mocked config.
struct ScopedRoutesConfigProvider : public Config::ConfigProvider {
  ScopedRoutesConfigProvider(TimeSource& time_source)
      : config_(std::make_shared<Router::MockScopedConfig>()), time_source_(time_source) {}

  ~ScopedRoutesConfigProvider() override = default;
