...
struct ActiveTestRequest {
  ...
  ActiveTestRequest(Http1ConnPoolImplTest& parent, size_t client_index, Type type)
    ...
    uint64_t current_rq_total = parent_.cluster_->stats_.upstream_rq_total().value();
    ...
    if (type != Type::Pending) {
      EXPECT_EQ(current_rq_total + 1, parent_.cluster_->stats_.upstream_rq_total().value());
...
TEST_F(Http1ConnPoolImplTest, MaxPendingRequests) {
  ...
  EXPECT_EQ(1U, cluster_->stats_.upstream_rq_pending_overflow().value());
}
...
TEST_F(Http1ConnPoolImplTest, ConnectFailure) {
  ...
  EXPECT_EQ(1U, cluster_->stats_.upstream_cx_connect_fail().value());
  EXPECT_EQ(1U, cluster_->stats_.upstream_rq_pending_failure_eject().value());
}
...
TEST_F(Http1ConnPoolImplTest, ConnectTimeout) {
  ...
  EXPECT_EQ(0U, cluster_->stats_.upstream_rq_total().value());
  EXPECT_EQ(2U, cluster_->stats_.upstream_cx_connect_fail().value());
  EXPECT_EQ(2U, cluster_->stats_.upstream_cx_connect_timeout().value());
}
...
TEST_F(Http1ConnPoolImplTest, MaxConnections) {
  ...
  EXPECT_EQ(1U, cluster_->stats_.upstream_cx_overflow().value());
...
TEST_F(Http1ConnPoolImplTest, ConnectionCloseHeader) {
  ...
  EXPECT_EQ(0U, cluster_->stats_.upstream_cx_destroy_with_active_rq().value());
}
...
TEST_F(Http1ConnPoolImplTest, MaxRequestsPerConnection) {
  ...
  EXPECT_EQ(0U, cluster_->stats_.upstream_cx_destroy_with_active_rq().value());
  EXPECT_EQ(1U, cluster_->stats_.upstream_cx_max_requests().value());
}
...
TEST_F(Http1ConnPoolImplTest, ConcurrentConnections) {
  ...
  conn_pool_->expectAndRunUpstreamReady();
  r3.startRequest();
  EXPECT_EQ(3U, cluster_->stats_.upstream_rq_total().value());
  ...
  EXPECT_EQ(2U, cluster_->stats_.upstream_cx_destroy().value());
  EXPECT_EQ(2U, cluster_->stats_.upstream_cx_destroy_remote().value());
}
...
TEST_F(Http1ConnPoolImplTest, DrainCallback) {
  ...
  EXPECT_EQ(1U, cluster_->stats_.upstream_rq_total().value());
...
TEST_F(Http1ConnPoolImplTest, RemoteCloseToCompleteResponse) {
  ...
  EXPECT_EQ(1U, cluster_->stats_.upstream_cx_destroy().value());
  EXPECT_EQ(1U, cluster_->stats_.upstream_cx_destroy_remote().value());
}
...
TEST_F(Http1ConnPoolImplTest, PendingRequestIsConsideredActive) {
  ...
  EXPECT_EQ(1U, cluster_->stats_.upstream_cx_destroy().value());
  EXPECT_EQ(1U, cluster_->stats_.upstream_cx_destroy_local().value());
}
