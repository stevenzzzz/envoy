...
TEST(OverProvisioningFactorTest, LocalityPickChanges) {
  auto setUpHostSetWithOPFAndTestPicks = [](const uint32_t over_provisioning_factor, const uint32_t pick_0, const uint32_t pick_1) {
    HostSetImpl host_set(0, over_provisioning_factor);
    std::shared_ptr<MockClusterInfo> cluster_info{new NiceMock<MockClusterInfo>()};
    HostVector hosts{
      makeTestHost(cluster_info, "tcp://127.0.0.1:80"),
    ...
    // Healthy ratio: (1/2, 1).
    HostsPerLocalitySharedPtr healthy_hosts_per_locality = makeHostsPerLocality(
        {{hosts[0]}, {hosts[2]}});
    host_set.updateHosts(
        makeHostsFromHostsPerLocality(hosts_per_locality),
        makeHostsFromHostsPerLocality(healthy_hosts_per_locality),
        hosts_per_locality,
