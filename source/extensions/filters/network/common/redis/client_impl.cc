...
ClientImpl::ClientImpl(Upstream::HostConstSharedPtr host, Event::Dispatcher& dispatcher,
  ...
  host->cluster().stats().upstream_cx_total().inc();
  host->stats().cx_total_.inc();
  host->cluster().stats().upstream_cx_active().inc();
...
ClientImpl::~ClientImpl() {
  ...
  host_->cluster().stats().upstream_cx_active().dec();
...
void ClientImpl::onConnectOrOpTimeout() {
  ...
  if (connected_) {
    host_->cluster().stats().upstream_rq_timeout().inc();
    host_->stats().rq_timeout_.inc();
  } else {
    host_->cluster().stats().upstream_cx_connect_timeout().inc();
...
void ClientImpl::onData(Buffer::Instance& data) {
  try {
    ...
    host_->cluster().stats().upstream_cx_protocol_error().inc();
    ...
    if (err.size() == 3) {
      ...
      if (err[0] == RedirectionResponse::get().MOVED || err[0] == RedirectionResponse::get().ASK) {
        ...
        if (redirect_succeeded) {
          host_->cluster().stats().upstream_internal_redirect_succeeded_total().inc();
        } else {
          host_->cluster().stats().upstream_internal_redirect_failed_total().inc();
        }
      }
    }
    if (!redirected) {
...
ClientImpl::PendingRequest::PendingRequest(ClientImpl& parent, ClientCallbacks& callbacks,
  ...
  parent.host_->cluster().stats().upstream_rq_total().inc();
  parent.host_->stats().rq_total_.inc();
  parent.host_->cluster().stats().upstream_rq_active().inc();
  parent.host_->stats().rq_active_.inc();
}

ClientImpl::PendingRequest::~PendingRequest() {
  parent_.host_->cluster().stats().upstream_rq_active().dec();
