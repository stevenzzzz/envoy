...
void TcpStatsdSink::TlsSink::write(Buffer::Instance& buffer) {
  ...
  if (parent_.cluster_info_->stats().upstream_cx_tx_bytes_buffered().value() >
  ...
  if (!connection_) {
    ...
    connection_->setConnectionStats({parent_.cluster_info_->stats().upstream_cx_rx_bytes_total(),
                                     parent_.cluster_info_->stats().upstream_cx_rx_bytes_buffered(),
                                     parent_.cluster_info_->stats().upstream_cx_tx_bytes_total(),
                                     parent_.cluster_info_->stats().upstream_cx_tx_bytes_buffered(),
                                     &parent_.cluster_info_->stats().bind_errors(), nullptr});
    connection_->connect();
  }
