...
RingHashLoadBalancer::RingHashLoadBalancer(
    const PrioritySet& priority_set, ClusterInfo& cluster_info, Stats::Scope& scope,
    Runtime::Loader& runtime, Random::RandomGenerator& random,
    const absl::optional<envoy::config::cluster::v3::Cluster::RingHashLbConfig>& config,
    const envoy::config::cluster::v3::Cluster::CommonLbConfig& common_config)
    : ThreadAwareLoadBalancerBase(priority_set, cluster_info, runtime, random, common_config),
