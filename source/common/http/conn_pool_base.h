...
public:
  ...
  void initialize(Upstream::Host::CreateConnectionData& data, HttpConnPoolImplBase& parent) {
    ...
    codec_client_->setConnectionStats(
        {parent_.host()->cluster().stats().upstream_cx_rx_bytes_total(),
         parent_.host()->cluster().stats().upstream_cx_rx_bytes_buffered(),
         parent_.host()->cluster().stats().upstream_cx_tx_bytes_total(),
         parent_.host()->cluster().stats().upstream_cx_tx_bytes_buffered(),
         &parent_.host()->cluster().stats().bind_errors(), nullptr});
  }
