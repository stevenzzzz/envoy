...
ActiveTcpClient::ActiveTcpClient(Envoy::ConnectionPool::ConnPoolImplBase& parent,
  ...
  connection_->setConnectionStats({host->cluster().stats().upstream_cx_rx_bytes_total(),
                                   host->cluster().stats().upstream_cx_rx_bytes_buffered(),
                                   host->cluster().stats().upstream_cx_tx_bytes_total(),
                                   host->cluster().stats().upstream_cx_tx_bytes_buffered(),
                                   &host->cluster().stats().bind_errors(), nullptr});
