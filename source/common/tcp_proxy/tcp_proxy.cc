...
void Filter::readDisableUpstream(bool disable) {
  ...
  if (disable) {
    read_callbacks_->upstreamHost()
        ->cluster()
        .stats()
        .upstream_flow_control_paused_reading_total().inc();
  } else {
    read_callbacks_->upstreamHost()
        ->cluster()
        .stats()
        .upstream_flow_control_resumed_reading_total().inc();
  }
}
...
Network::FilterStatus Filter::establishUpstreamConnection() {
  ...
  if (!cluster->resourceManager(Upstream::ResourcePriority::Default).connections().canCreate()) {
    ...
    cluster->stats().upstream_cx_overflow().inc();
  ...
  if (connect_attempts_ >= max_connect_attempts) {
    ...
    cluster->stats().upstream_cx_connect_attempts_exceeded().inc();
  ...
  if (!maybeTunnel(*thread_local_cluster)) {
    ...
    // cluster->stats().upstream_cx_none()healthy in the latter case.
