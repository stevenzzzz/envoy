...
std::unique_ptr<ScopeKeyFragmentBase> HeaderValueExtractorImpl::computeFragment(const Http::HeaderMap& headers) const  {
  const Envoy::Http::HeaderEntry* header_entry = headers.get(Envoy::Http::LowerCaseString(header_value_extractor_config_.name()));
  if(header_entry==nullptr) {return nullptr; }
  ...
  if(header_value_extractor_config_.element_separator().length()>0){
    elements = absl::StrSplit(header_entry->value().getStringView(), header_value_extractor_config_.element_separator());
  }
  switch(header_value_extractor_config_.extract_type_case()) {
    case ScopedRoutes::ScopeKeyBuilder::FragmentBuilder::HeaderValueExtractor::kElement:
      for (const auto& element: elements ) {
        std::pair<absl::string_view, absl::string_view> key_value = absl::StrSplit(element, absl::MaxSplits(header_value_extractor_config_.element().separator(),1));
        if(key_value.first==header_value_extractor_config_.element().key()){
          return absl::make_unique<StringKeyFragment>(key_value.second);
        }
      }
      break;
    case ScopedRoutes::ScopeKeyBuilder::FragmentBuilder::HeaderValueExtractor::kIndex:
      if (header_value_extractor_config_.index() < elements.size()) {
        return absl::make_unique<StringKeyFragment>(elements[header_value_extractor_config_.index()]);
      }
      break;
    default:// EXTRACT_TYPE_NOT_SET
      ENVOY_LOG_MISC(error, "HeaderValueExtractor extract_type not set: {}.", header_value_extractor_config_.DebugString());
      return nullptr;
  }
...
ScopeKeyBuilderImpl::ScopeKeyBuilderImpl(
    ScopedRoutes::ScopeKeyBuilder config):ScopeKeyBuilderBase (config) {
    for (const auto& fragment_builder: config_.fragments()) {
      switch(fragment_builder.type_case()){
        case ScopedRoutes::ScopeKeyBuilder::FragmentBuilder::kHeaderValueExtractor:
          fragment_builders_.emplace_back(absl::make_unique<HeaderValueExtractorImpl>(fragment_builder));
          break;
        default:
...
const ScopeKey ScopeKeyBuilderImpl::computeScopeKey(const Http::HeaderMap& headers) const{
     ...
     for(const auto&builder :fragment_builders_){
       key.emplace_back(builder->computeFragment(headers));
     }
     return key;
}
