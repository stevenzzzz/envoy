...
Http::FilterHeadersStatus Filter::decodeHeaders(Http::RequestHeaderMap& headers, bool end_stream) {
  ...
  if (cluster_->maintenanceMode()) {
    ...
    cluster_->stats().upstream_rq_maintenance_mode().inc();
    return Http::FilterHeadersStatus::StopIteration;
  }
...
Http::FilterDataStatus Filter::decodeData(Buffer::Instance& data, bool end_stream) {
  ...
  if (buffering &&
    ...
    ENVOY_LOG(debug,
              ...
              getLength(callbacks_->decodingBuffer()) + data.length(), retry_shadow_buffer_limit_);
    cluster_->stats().retry_or_shadow_abandoned().inc();
    retry_state_.reset();
    buffering = false;
...
void Filter::onResponseTimeout() {
  ...
  while (!upstream_requests_.empty()) {
    ...
    if (Runtime::runtimeFeatureEnabled(
      ...
      cluster_->stats().upstream_rq_timeout().inc();
...
void Filter::onPerTryIdleTimeout(UpstreamRequest& upstream_request) {
  onPerTryTimeoutCommon(upstream_request, cluster_->stats().upstream_rq_per_try_idle_timeout(),
                        StreamInfo::ResponseCodeDetails::get().UpstreamPerTryIdleTimeout);
}

void Filter::onPerTryTimeout(UpstreamRequest& upstream_request) {
  onPerTryTimeoutCommon(upstream_request, cluster_->stats().upstream_rq_per_try_timeout(),
...
bool Filter::setupRedirect(const Http::ResponseHeaderMap& headers) {
  ...
  if (downstream_end_stream_ && (!request_buffer_overflowed_ || !callbacks_->decodingBuffer()) &&
    ...
    cluster_->stats().upstream_internal_redirect_succeeded_total().inc();
    return true;
  }
  ...
  cluster_->stats().upstream_internal_redirect_failed_total().inc();
  return false;
}
