...
void UpstreamRequest::cleanUp() {
  ...
  while (downstream_data_disabled_ != 0) {
    ...
    parent_.cluster()->stats().upstream_flow_control_drained_total().inc();
...
void UpstreamRequest::onStreamMaxDurationReached() {
  upstream_host_->cluster().stats().upstream_rq_max_duration_reached().inc();
...
void UpstreamRequest::DownstreamWatermarkManager::onAboveWriteBufferHighWatermark() {
  ...
  parent_.parent_.cluster()->stats().upstream_flow_control_paused_reading_total().inc();
...
void UpstreamRequest::DownstreamWatermarkManager::onBelowWriteBufferLowWatermark() {
  ...
  parent_.parent_.cluster()->stats().upstream_flow_control_resumed_reading_total().inc();
...
void UpstreamRequest::disableDataFromDownstreamForFlowControl() {
  ...
  parent_.cluster()->stats().upstream_flow_control_backed_up_total().inc();
